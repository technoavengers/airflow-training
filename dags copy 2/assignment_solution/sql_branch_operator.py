...
with DAG(dag_id="sql_branch_operator_assignment", schedule_interval="@daily", default_args=default_args,tags=['assignment']) as dag:
    ...
    is_user_added = BranchSQLOperator(
        task_id="is_user_added",
        conn_id="postgres",
        sql="SELECT count(1) FROM users",
        follow_task_ids_if_true="user_added",
        follow_task_ids_if_false="user_not_added",
        dag=dag
        )
   

    user_added = DummyOperator(
            task_id='user_added',
     )
        
    user_not_added = DummyOperator(
            task_id='user_not_added',
     )
    ...
    get_user >> process_user >> create_table >> insert_data >> is_user_added >> [user_added,user_not_added] >> notify